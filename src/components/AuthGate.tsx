...
export default function AuthGate({ onAuthed }: { onAuthed: (name: string) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch(`/api/auth/${mode}`, {
        ...
        body: JSON.stringify(mode === "register" ? { username, password, name } : { username, password }),
      });
  ...
  return (
    <main className="max-w-md mx-auto px-4 py-20">
          ...
          {(["login", "register"] as const).map((m) => (
            <button
              ...
              className={`px-3 py-1 text-sm rounded-sm transition-colors ${
                mode === m ? "bg-gold/15 text-gold border border-gold/40" : "text-faded hover:text-cream"
              }`}
            >
              {m === "login" ? "登入" : "註冊仙籍"}
            </button>
        ...
        <div className="space-y-3">
          {mode === "register" && (
            <div>
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="韓立" maxLength={12}
          ...
          <div>
            ...
            <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="account" maxLength={32}
              className="w-full bg-smoke border border-faded/30 rounded-sm px-3 py-2 text-parchment placeholder-faded/40 focus:outline-none focus:border-gold/60" />
          </div>
          <div>
            <p className="text-xs text-faded mb-1">密碼</p>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="至少 4 位"
      ...
      <p className="text-center text-xs text-faded/60 mt-6">註冊後,仙體每一小時自然增壽一年(離線亦然)。</p>
    </main>
  );
}
