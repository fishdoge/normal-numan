...
export interface Technique {
  ...
  // 進階連攜特性(3.13 版新增,見 pvp-territory-design.md 第九節):目前只掛在精選的中高階仙法上,
  // 不是每張都有,刻意保留一部分「單純比倍率」的基礎牌當對照組。
  comboSizePct?: number; // 本次出牌批次每多打一張(不分元素),此符寶疊加此比例傷害,例:0.15 = 每多一張 +15%
  crossElementPct?: number; // 批次中每有一種跟自己不同的元素(不分張數,同元素只算一次),疊加此比例傷害
  manaGain?: number; // 打出後立即回復的法力(可部分或全額抵銷自身消耗,甚至淨賺法力)
  endsTurn?: boolean; // 打出後立即觸發回合結束,不需要玩家再按施法結束,適合當終結技
  shieldPct?: number; // 打出後立即獲得「氣血上限 × 此比例」護盾,沿用戰術卡護體法箓的護盾機制
  forceStatus?: "burn" | "poison" | "freeze"; // 打出後必定對敵方附加此狀態,不受一般命中機率限制
  ...
  // ── 3.14 版新增:威力倍率大幅壓平的同時,補上四種新連攜機制,讓可組合性不再只靠數字堆疊 ──
  // 雙/三屬性符寶:除了主屬性(element)外同時視為這裡列出的屬性(1~2 個),五行相剋取全部屬性中
  // 最高倍率,亦全數計入同批次的 elementCounts(讓其他符寶的 synergyPct/crossElementPct 判定
  // 把這張也算進去)。天生百搭,適合彌補屬性剋制的空缺。
  extraElements?: Element[];
  // 對自身生效的狀態(目前僅 evade):打出後為玩家附加一次「必定避過怪物下一次攻擊」的效果,
  // 用掉即消,不與命中機率式的閃避疊加判定。
  selfStatus?: "evade";
  // 蓄力:此符寶本身幾乎不造成傷害,但會登記到下一整個玩家回合——那一整回合內打出的所有符寶,
  // 傷害總和額外提升這個比例。慣例上搭配 soloOnly + endsTurn 使用(這回合只能打這張、打完立即
  // 結束回合),對應「這回合不能攻擊,下回合攻擊加倍」的蓄力型卡牌。
  chargeNextTurnPct?: number;
  // 多段傷害:總傷害計算方式不變,但拆成這麼多段個別數字連續命中,呈現「連續打擊」的觀感,
  // 也因此更容易在浮動傷害數字上看出差異。設計慣例上搭配 soloOnly 使用。
  hits?: number;
  // 清心:打出後立即清除玩家身上所有負面狀態(燒傷/中毒/冰封/虛弱),不影響 evade 這種增益型狀態。
  selfCleanse?: boolean;
  // 吸血:打出後依這次造成的傷害的此比例回復氣血(不超過氣血上限)。
  lifeStealPct?: number;
}
